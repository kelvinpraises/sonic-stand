...
// IPFS Gateway with token
const IPFS_GATEWAY = "https://lime-hidden-rodent-657.mypinata.cloud/ipfs";
const PINATA_TOKEN = "pinataGatewayToken=8jiuhbdADzKIP0YFFfTybOIUUXvRMq0E7nHuzlm8qDo0Ri6euvHa7xiPVDvbODVf";

// Helper to get full IPFS URL
const getIPFSUrl = (cid: string): string => {
  return `${IPFS_GATEWAY}/${cid}?${PINATA_TOKEN}`;
};
                  ...
                  >
                    ...
                    {active.scenes && active.scenes.length > 0 && (
                      <div className="w-full">
                              ...
                              {active.scenes.map((scene, idx) => (
                                ...
                                >
                                  <Image
                                    width={120}
                                    height={80}
                                    src={active.capturedImages && idx < active.capturedImages.length 
                                      ? getIPFSUrl(active.capturedImages[idx]) 
                                      : getIPFSUrl(active.cover)}
                                    alt={`Scene ${idx + 1}`}
                                  ...
                                  <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-md">
                                    <span className="text-white text-xs font-medium">Scene {idx + 1}</span>
                                  </div>
                                </div>
                              ))}
                            </div>
                        ...
                        {active.scenes.map((scene, idx) => (
                          <div key={idx} className="mb-3 pb-3 border-b border-gray-200 dark:border-gray-700">
            ...
            >
              ...
              <div className="w-full h-full bg-black flex items-center justify-center">
                <Image
                  ...
                  height={600}
                  src={
                    active.capturedImages && selectedImageIndex < active.capturedImages.length
                      ? getIPFSUrl(active.capturedImages[selectedImageIndex])
                      : getIPFSUrl(active.cover)
                  }
                  alt={selectedImageIndex !== null ? `Scene ${selectedImageIndex + 1}` : "Preview"}
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-4">
                <div className="line-clamp-2">
                  <p className="text-white text-sm">
                    {selectedImageIndex !== null && active.scenes && active.scenes[selectedImageIndex] 
                      ? `Scene ${selectedImageIndex + 1}: ${active.scenes[selectedImageIndex].description}` 
